...
const webpackConfig = merge(baseWebpackConfig, {
  module: {
    rules: utils.styleLoaders({
      sourceMap: config.build_test.testSourceMap,
      extract: true,
      usePostCSS: true
    })
  },
  devtool: config.build_test.testSourceMap ? config.build_test.devtool : false,
  output: {
  ...
  plugins: [
    ...
    new OptimizeCSSPlugin({
      cssProcessorOptions: config.build_test.testSourceMap
...
if (config.build_test.testGzip) {
  ...
  webpackConfig.plugins.push(
    new CompressionWebpackPlugin({
      ...
      algorithm: 'gzip',
      test: new RegExp(
        '\\.(' + config.build_test.testGzipExtensions.join('|') + ')$'
      ),
      threshold: 10240,
      minRatio: 0.8
    })
